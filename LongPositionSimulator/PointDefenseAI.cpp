                              ...
                              double currentTime, double updateDelta)
{
    ...
    {
        totalAcc = totalAcc + missile.positionable.acceleration;
    }
    totalAcc = totalAcc / enemyMissiles.size();
    //thrust away from missiles
    auto updatedSelf = SpaceObject(thisShip.objectAt(currentTime));
    updatedSelf.positionable.acceleration = totalAcc.normalized() * 10; // FIXME use ship thrust here.
    ...
    for (int i = 0; i < 20; i++)
    {
        for (auto& missile : enemyMissiles)
        {
            ...
            if (!missileHitColl) //ignore missiles that will miss
                continue;
            if (missile.positionable.distanceToObjectAt(currentTime, thisShip.positionable) > PD_TURRET_RANGE)
