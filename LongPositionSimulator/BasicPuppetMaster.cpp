...
void BasicPuppetMaster::onNewObject(SpaceObject obj) const
{
    ...
    if (obj.meta.type == ObjectType::PROJECTILE && obj.meta.faction == OpFor)
    {
        ...
        if (blushipMaybe)
        {
            ...
            auto evadeVector = vel3d::crossProduct(bluship.positionable.acceleration.normalized(),
                                                   obj.positionable.velocity.normalized());
            bluship.positionable.acceleration = evadeVector * 10; // upwards thrust
...
void BasicPuppetMaster::onUpdate(const SpaceObject& obj)
{
    ...
    if (obj.globalObjectId == REDSHIP_ID)
    {
        ...
        if (clickedX != 0)
        {
            objectNow.positionable.acceleration = acc3d{
                clickedX, clickedY, 0
            }.normalized() * 20;
            this->clickedX = 0;
                    ...
                    case Weapon::RAILGUN:
                        {
                            for (int j = 0; j < 2; j++) // fire a burst at target
                            {
                                ...
                                if (bullet)
                                {
                                    bullet.value().positionable.radius = 1;
...
void BasicPuppetMaster::onCollissionHappened(CollissionPoint coll) const
{
    ...
    std::cout << "Collision happened at time: " << coll.time << " between " << coll.obj1 << " and " << coll.obj2 <<
        "\n";
    std::cout << " distance at collission:" << coll.obj1.positionable.distanceToObjectAt(
        coll.time, coll.obj2.positionable) << "\n";
    std::cout << " position of collission:\n" << coll.obj1 << ": " << coll.obj1.positionable.posAt(coll.time) << " vs "
        << coll.obj2 << ": " << coll.obj2.positionable.posAt(coll.time) << std::endl;
}
